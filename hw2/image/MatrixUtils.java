...
public class MatrixUtils {
    ...
    public static double[][] accumulateVertical(double[][] m) {
        ...
        for (int r = 1; r < copy.length; r++) {
            for (int c = 0; c < copy[0].length; c++) {
                double minVal = Math.min(getHelper(copy, r-1, c-1), Math.min(getHelper(copy, r-1, c), getHelper(copy, r-1, c+1)));
                copy[r][c] += minVal;
            }
        }
        return copy;
    ...
    private static double getHelper(double[][] e, int r, int c) {
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        if (orientation == Orientation.HORIZONTAL) {
           return transposeHelper(accumulateVertical(transposeHelper(m)));
        }
        return accumulateVertical(m);
    }

    private static double[][] transposeHelper(double[][] m) {
        int r = m[0].length;
        int c = m.length;
