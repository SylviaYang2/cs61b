...
public class ArrayHeap<T> {
    ...
    private void bubbleUp(int index) {
        while (index > 1 && getNode(index).priority() < getNode(getParentOf(index)).priority()) {
...
//        }
        ...
        } else if (getNode(right) == null && getNode(left) != null) {
            if (getNode(index).priority() > getNode(left).priority()) {
                swap(index, left);
            }
        } else if (getNode(left) == null && getNode(right) == null) {
            return;
        } else {
            if (getNode(left).priority() <= getNode(right).priority()) {
                min = left;
            } else {
                min = right;
            }
