...
class Model {
    ...
    boolean[] colorsPresent() {
        boolean[] colorPresent = new boolean[_ncolors];
        for (int r = 0; r < _height; r++) {
            for (int c = 0; c < _width; c++) {
                for (int num = 0; num < _ncolors; num++) {
                    if (get(r, c) == num) {
                        colorPresent[num] = true;
                    }
                }
            }
        }
    ...
    Set<Place> adjacentCells(int color) {
        ...
        HashSet<Place> copyOfActive = new HashSet<>(_active);
        undo();
        copyOfActive.removeAll(_active);
        return copyOfActive;
    }
